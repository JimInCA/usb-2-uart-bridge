...
#define USB_UART_CLKSRC     UART3_CLK_SRC
#define USB_UART_CLK_FREQ   CLOCK_GetFreq(UART3_CLK_SRC)
#define USB_UART_IRQn       UART3_RX_TX_IRQn
#define USB_UART_IRQHandler UART3_RX_TX_IRQHandler
...
uint8_t USB_demoRingBuffer[UART_RING_BUFFER_SIZE];
...
void USB_UART_IRQHandler(void)
{
    ...
    if ((kUART_RxDataRegFullFlag | kUART_RxOverrunFlag) & UART_GetStatusFlags(USB_UART))
    {
        ...
        if (((USB_rxIndex + 1) % UART_RING_BUFFER_SIZE) != USB_txIndex)
        {
            USB_demoRingBuffer[USB_rxIndex] = data;
            USB_rxIndex++;
            USB_rxIndex %= UART_RING_BUFFER_SIZE;
        }

        if (USB_rxIndex)
        {
            usb_status_t error = kStatus_USB_Error;
            uint32_t size = USB_rxIndex;
            USB_rxIndex    = 0;

            error = USB_DeviceCdcAcmSend(s_cdcVcom.cdcAcmHandle, USB_CDC_VCOM_BULK_IN_ENDPOINT, USB_demoRingBuffer, size);
    ...
    switch (event)
    {
        ...
        case kUSB_DeviceCdcEventRecvResponse:
        {
            if ((1 == s_cdcVcom.attach) && (1 == s_cdcVcom.startTransactions))
            {
                ...
                {
                    UART_WriteBlocking(USB_UART, s_currRecvBuf, s_recvSize);
                    s_recvSize = 0;
                    // I don't know why I need to make the following call.
                    // But this bridge won't work without it.
                    // This is something that needs to be resolved in the future.
                    error = USB_DeviceCdcAcmSend(s_cdcVcom.cdcAcmHandle, USB_CDC_VCOM_BULK_IN_ENDPOINT, s_currRecvBuf, s_recvSize);
                }
            }
        }
        break;
 ...
 * @return None.
 */
void BOARD_InitUSB2UARTBridge(void)
{
    ...
    UART_Init(USB_UART, &config, USB_UART_CLK_FREQ);
    ...
    //UART_WriteBlocking(USB_UART, g_tipString, sizeof(g_tipString) / sizeof(g_tipString[0]));
